...
describe('saveSegment', () => {
	it('saves and retrieves a segment by id', async () => {
		const id = await saveSegment(blob(), 'audio/webm', 1000, 1010, MONITOR_A);
	...
	it('enforces rolling buffer: total unpinned per monitor stays ≤ MAX_ROLLING_SEGMENTS', async () => {
		for (let i = 0; i < MAX_ROLLING_SEGMENTS + 3; i++) {
			await saveSegment(blob(), 'audio/webm', i * 10, (i + 1) * 10, MONITOR_A);
		}
	...
	it('does not evict segments from other monitors', async () => {
		await saveSegment(blob(), 'audio/webm', 0, 10, MONITOR_B);
	...
	it('does not evict pinned segments', async () => {
		...
		const id1 = await saveSegment(blob(), 'audio/webm', 0, 10, MONITOR_A);
		const id2 = await saveSegment(blob(), 'audio/webm', 10, 20, MONITOR_A);
		await pinRange(0, 20);

		// Flood with new unpinned segments
		for (let i = 3; i < MAX_ROLLING_SEGMENTS + 5; i++) {
			await saveSegment(blob(), 'audio/webm', i * 100, (i + 1) * 100, MONITOR_A);
		}
...
describe('pinRange', () => {
	...
	it('returns all segments overlapping the range', async () => {
		await saveSegment(blob(), 'audio/webm', 0, 10, MONITOR_A);   // overlaps [5,25]
		await saveSegment(blob(), 'audio/webm', 10, 20, MONITOR_A);  // overlaps
		await saveSegment(blob(), 'audio/webm', 20, 30, MONITOR_A);  // overlaps (endTime=30 > from=5, startTime=20 <= to=25)
	...
	it('marks matched segments as pinned in IDB', async () => {
		const id = await saveSegment(blob(), 'audio/webm', 0, 10, MONITOR_A);
		await pinRange(0, 10);
		const seg = await getSegmentById(id);
	...
	it('does not pin segments outside the range', async () => {
		const id = await saveSegment(blob(), 'audio/webm', 100, 110, MONITOR_A);
		await pinRange(0, 50);
		const seg = await getSegmentById(id);
		expect(seg!.pinned).toBe(false);
	});
...
describe('getSegmentsInRange', () => {
	it('returns segments whose time window overlaps the query range', async () => {
		await saveSegment(blob(), 'audio/webm', 0, 10, MONITOR_A);
		await saveSegment(blob(), 'audio/webm', 10, 20, MONITOR_A);
		await saveSegment(blob(), 'audio/webm', 50, 60, MONITOR_A);
	...
	it('returns results sorted by startTime', async () => {
		await saveSegment(blob(), 'audio/webm', 20, 30, MONITOR_A);
		await saveSegment(blob(), 'audio/webm', 0, 10, MONITOR_A);
		await saveSegment(blob(), 'audio/webm', 10, 20, MONITOR_A);
...
describe('getCoverageMap', () => {
	...
	it('produces separate ranges when there is a gap', async () => {
		await saveSegment(blob(), 'audio/webm', 0, 10, MONITOR_A);
		await saveSegment(blob(), 'audio/webm', 20, 30, MONITOR_A); // gap at [10,20]
...
describe('getStorageUsed', () => {
	...
	it('returns sum of sizeBytes for matching segments', async () => {
		...
		await saveSegment(b1, 'audio/webm', 0, 10, MONITOR_A);
		await saveSegment(b2, 'audio/webm', 10, 20, MONITOR_A);
	...
	it('filters by monitor when provided', async () => {
		...
		await saveSegment(b, 'audio/webm', 0, 10, MONITOR_A);
		await saveSegment(b, 'audio/webm', 0, 10, MONITOR_B);
